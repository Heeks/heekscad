...
void CSelectMode::OnMiddleDown( wxMouseEvent& event )
{
...
void CSelectMode::OnMoving( wxMouseEvent& event )
{
	CurrentPoint = wxPoint(event.GetX(), event.GetY());

	if(wxGetApp().m_mouse_move_highlighting)
	{
		m_highlighted_objects.clear();
...
void CSelectMode::OnWheelRotation( wxMouseEvent& event )
{
