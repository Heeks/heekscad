...
void PointOrWindow::OnMouse( wxMouseEvent& event ){
	if(event.LeftDown()){
		...
		if(event.LeftIsDown() && !use_window && window && abs(event.GetX() - mouse_down_point.x)>2 && abs(event.GetY() - mouse_down_point.y)>2){
			...
			box_chosen.y = wxGetApp().m_frame->m_graphics->GetSize().GetHeight() - mouse_down_point.y;
			window->window_box = box_chosen;
		}

		if(use_window){
			box_chosen.width = event.GetX() - box_chosen.x;
			box_chosen.height = wxGetApp().m_frame->m_graphics->GetSize().GetHeight() - event.GetY() - box_chosen.y;
		}
		else{
...
void PointOrWindow::SetWithPoint(const wxPoint &point){
	...
	box_chosen.y = wxGetApp().m_frame->m_graphics->GetSize().GetHeight() - point.y - 5;
	box_chosen.width = 10;
...
void PointOrWindow::OnFrontRender(){
	...
	if(!use_window)
	{
		...
		glPushMatrix();
		wxGetApp().m_frame->m_graphics->SetIdentityProjection();
		wxGetApp().m_frame->m_graphics->DrawWindow(box_chosen, false);
		glMatrixMode(GL_MODELVIEW);
		glPopMatrix();
		glMatrixMode(GL_PROJECTION);
		glPopMatrix();
