...
void MagDragWindow::OnMouse( wxMouseEvent& event )
{
	...
	else if(event.LeftUp())
	{
		...
		window_box.height = (wxGetApp().m_frame->m_graphics->GetSize().GetHeight() - event.GetY()) - window_box.y;
	...
	else if(event.Dragging())
	{
		...
		if(window_box_exists)wxGetApp().m_frame->m_graphics->DrawWindow(window_box, false);// undraw the window
		window_box.width = event.GetX() - window_box.x;
		window_box.height = (wxGetApp().m_frame->m_graphics->GetSize().GetHeight() - event.GetY()) - window_box.y;
		wxGetApp().m_frame->m_graphics->DrawWindow(window_box, false);// draw the window
...
void MagDragWindow::OnFrontRender(){
	if(window_box_exists){
		wxGetApp().m_frame->m_graphics->DrawWindow(window_box, false);
	}
}
