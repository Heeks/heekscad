...
bool RS274X::Read( const char *p_szFileName, const FileInterpretation_t file_interpretation, const bool force_mirror_on /* = false */ )
{
	...
	if (input.is_open())
	{
		...
		if (file_interpretation == RasterImage)
        {
            ...
            {
                wxString file_name;
                file_name << wxT("pcb") << wxT("_width_") << pcb.PixelsPerRow() << wxT("_height_") << pcb.PixelsPerColumn() << wxT(".raw");
                pcb.Save( file_name );
            }
        } // End if - then
...
bool RS274X::Bitmap::Save( const wxString file_name ) const
{
	...
	// output_file_name << wxT(".gif");

	wxString script_file_name;
	script_file_name << output_file_name << wxT(".gif");
	...
	if (fp != NULL)
	{
		...
		contents << wxT("#!/bin/bash\n\n");
		contents << wxT("# This file converts the RAW image file generated by the HeeksCAD import of\n");
		contents << wxT("# a GERBER format data file into a GIF format file.   This just makes it\n");
		contents << wxT("# more convenient for viewing\n\n");
		contents << wxT("rawtoppm < \"") << file_name << wxT("\" ") << PixelsPerRow() << wxT(" ") << PixelsPerColumn() << wxT(" | ppmtogif > \"") << output_file_name << wxT(".gif") << wxT("\"\n");
