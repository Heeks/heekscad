...
void CSelectMode::OnMouse( wxMouseEvent& event )
{
		...
		if(marked_object.m_map.size()>0)
		{
			...
			while(object)
			{
				if(object->GetType() == GripperType)
				{
					...
					extract(wxGetApp().m_digitizing->digitized_point.m_point, grip_from);
					memcpy(grip_to, grip_from, 3*sizeof(double));
					wxGetApp().drag_gripper->OnGripperGrabbed(wxGetApp().m_marked_list->list(), true, grip_from);
	...
	if(event.LeftUp())
	{
		...
		{
			wxGetApp().drag_gripper->OnGripperReleased(grip_from, grip_to);
	...
	else if(event.Dragging())
	{
		...
		else if(event.LeftIsDown())
		{
			if(wxGetApp().drag_gripper)
			{
				...
				extract(wxGetApp().m_digitizing->digitized_point.m_point, grip_to);
				wxGetApp().drag_gripper->OnGripperMoved(grip_from, grip_to);
			}
			else if(abs(button_down_point.x - event.GetX())>2 || abs(button_down_point.y - event.GetY())>2)
			{
				if(!window_box_exists && m_can_grip_objects)
				{
					...
					if(selected_objects_dragged.size() > 0)
					{
						...
						extract(wxGetApp().m_digitizing->digitized_point.m_point, grip_from);
						memcpy(grip_to, grip_from, 3*sizeof(double));
						wxGetApp().drag_gripper->OnGripperGrabbed(selected_objects_dragged, wxGetApp().m_show_grippers_on_drag, grip_from);
						wxGetApp().m_digitizing->digitize(wxPoint(event.GetX(), event.GetY()));
						extract(wxGetApp().m_digitizing->digitized_point.m_point, grip_to);
						wxGetApp().drag_gripper->OnGripperMoved(grip_from, grip_to);
						return;
